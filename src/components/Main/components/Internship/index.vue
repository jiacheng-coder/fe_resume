<template>
  <div v-if="resume.internship.length" ref="rootRef" class="mb-6">
    <h1 class="mb-4 text-3xl text-gray-700 font-bold">{{ resume.ui.sections.internship }}</h1>
    <ul>
      <li class="intern-company mb-4" v-for="item in resume.internship" :key="item.id || item.company">
        ...
        <div class="intern-block resume-project-block mb-2" v-for="project in item.projects" :key="project.id || project.name">
...
onMounted(() => {
  nextTick(() => {
    const companies = rootRef.value?.querySelectorAll('.intern-company')
    ...
    const blocks = rootRef.value?.querySelectorAll('.intern-block')
